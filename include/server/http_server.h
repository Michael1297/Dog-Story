...
public:
    ...
    void Write(http::response<Body, Fields>&& response){
        ...
        auto response_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - request_time).count();
...
private:
    ...
    void HandleRequest(HttpRequest&& request) override {
        ...
        json::value request_data {
                {"ip", GetEndpoint().address().to_string()},
                {"URI", GetRequest().target()},
                {"method", http::to_string(GetRequest().method())}
        };
